<?php
/**
 * IBSng Provisioning Module for WHMCS
 * Updated for WHMCS 8.13.1 compatibility.
 * - Replaced deprecated mysql_query/update_query calls with Capsule ORM updates.
 * - Added `use WHMCS\Database\Capsule;` to use the WHMCS database layer.
 * - Left business logic untouched; only modernized DB interactions.
 *
 * Note: Password values are stored via WHMCS `encrypt()` helper as before.
 * Tested against PHP 8.1+/WHMCS 8.13.1.
 */
...
function ibsng_ConfigOptions() {
    $configarray = array(
        "نام گروه" => array(
            "Type" => "text",
            "Size" => "25",
            "Description" => "یک گروه در IBSng ایجاد کرده و نام آن را وارد کنید"
        ),
        "پیشوند نام کاربری" => array(
            "Type" => "text",
            "Size" => "25",
            "Description" => "پیشوند دلخواه برای نام کاربری ( مثال : uk- )"
        ),
        "طول نام کاربری" => array(
            "Type" => "text",
            "Size" => "10",
            "Description" => "پیشفرض : 8"
        ),
        "طول کلمه عبور" => array(
            "Type" => "text",
            "Size" => "10",
        ...
        "ترکیب نام کاربری" => array(
            "Type" => "dropdown",
            "Options" => "LowercaseAlphabet,UppercaseAlphabet,Numeric,AlphabetNumeric"
        ),
        "ترکیب کلمه عبور" => array(
            "Type" => "dropdown",
            "Options" => "Numeric,LowercaseAlphabet,UppercaseAlphabet,AlphabetNumeric"
        ),
        "دسترسی کاربران به پنل Internet" => array(
            "Type" => "yesno",
            "Description" => "دسترسی کاربران جهت ورود به پنل Internet"
        ),
        "دسترسی کاربران به پنل VOIP" => array(
            "Type" => "yesno",
            "Description" => "دسترسی کاربران جهت ورود به پنل VOIP"
        ),
        "نمایش وضعیت اکانت" => array(
            "Type" => "yesno",
            "Description" => "نمایش وضعیت آنلاین / آفلاین بودن اکانت"
        ),
        "امکان تغییر کلمه عبور" => array(
            "Type" => "yesno",
            "Description" => "دسترسی کاربران جهت تغییر کلمه عبور اکانت"
        ),
        "مقدار اعتبار ( Credit )" => array(
            "Type" => "text",
            "Size" => "10",
            "Description" => "UNITS"
        ),
        "نمایش مقدار اعتبار ( Credit )" => array(
            "Type" => "yesno",
            "Description" => "نمایش مقدار اعتبار ( Credit ) در پنل کاربری"
        ),
        "واحد اعتبار ( Credit )" => array(
            "Type" => "text",
            "Size" => "10",
            "Description" => "واحد نمایشی اعتبار در پنل کاربری"
        ),
    );
...
function random($length, $type) {
    if (empty($length) || $length == 0) $length = 8;
    if (empty($type)) $type = "AlphabetNumeric";
...
function ibsng_ClientArea($params) {
    ...
    if ($ibsng_access_status == true) {
        $code .= '<tr>
            ...
            userStatusCheck($params['serverusername'], $params['serverpassword'], $params['serverip'], $params['username']) .
            '</td>
        </tr>';
    }
    ...
    if ($ibsng_access_credit == true) {
        $code .= '<tr>
            ...
            <td style="width:50%; text-align:right; direction:rtl;">' .
            userGetCredit($params['username'], $params['password'], $params['serverip'], $params['username']) . ' ' . $params['configoption13'] .
            '</td>
        </tr>';
    }
    ...
    if ($ibsng_access_i == true || $ibsng_access_v == true) {
        $code .= "<table class='table table-bordered'><tbody><tr>";
    ...
    if ($ibsng_access_change_pw == true) {
        ...
        if (isset($_POST['do']) && $_POST['do'] == 'IBSngChangePassword') {
            if (empty($_POST['cpOld']) || empty($_POST['cpNew']) || empty($_POST['cpRNew'])) {
            ...
            } else if ($_POST['cpNew'] !== $_POST['cpRNew']) {
                ...
                try {
                    ...
                    if ($command === true || (is_string($command) && strtolower($command) === "success")) {
                        ...
                        $code .= "<div class='alert alert-danger'>خطا در IBSng: " . htmlspecialchars(print_r($command, true)) . "</div>";
                    }
                } catch (\Exception $e) {
                    $code .= "<div class='alert alert-danger'>خطا: " . htmlspecialchars($e->getMessage()) . "</div>";
                }
            }
        }
            ...
            <table class="table table-bordered">
                <tbody>
                    <tr>
                        ...
                        <td style="width:75%; text-align:left; direction:ltr;"><input type="text" class="form-control" name="cpOld" value="' . htmlspecialchars($params['password']) . '"></td>
                    </tr>
                    <tr>
